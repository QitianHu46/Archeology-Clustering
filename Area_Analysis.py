class AreaAnalysis2:
    ...
    def __init__(self, labels, ctr_points, areas):
        ...
        self.Results.number_of_buildings = [ len([ i for i in range ]) for i in range(num_clusters)]

        # add data for cluster_number, number_of_buildings, total_area
        Results.number_of_buildings = [len(self.areas[dfArea.cluster == i]) for i in range(num_clusters) ]
